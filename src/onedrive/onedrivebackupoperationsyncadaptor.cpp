...
{
    QDBusPendingReply<QString> backupDeviceIdReply = m_sailfishBackup->asyncCall("backupFileDeviceId");
    backupDeviceIdReply.waitForFinished();
    ...
    case Backup:
    {
        QDBusPendingReply<QString> createBackupReply =
                m_sailfishBackup->asyncCall("createBackupForSyncProfile", m_accountSyncProfile->name());
        createBackupReply.waitForFinished();
        if (createBackupReply.isError() || createBackupReply.value().isEmpty()) {
...
void OneDriveBackupOperationSyncAdaptor::listOperationFinished()
{
    ...
    QDBusPendingReply<void> setCloudBackupsReply =
            m_sailfishBackup->asyncCall("setCloudBackups", m_accountSyncProfile->name(), dirListing);
    setCloudBackupsReply.waitForFinished();
